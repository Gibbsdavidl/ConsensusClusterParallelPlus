ConsensusClusterParallelPlus <- function( d=NULL,
                                  maxK = 3,
                                  reps=10,
                                  cores=1,
                                  pItem=0.8,
...
ccRun <- function( d=d,
  ...
  require(parallel)
  parRes <- mclapply(X=1:repCount, FUN=function(xi) {eachRep(xi,d, pItem, pFeature,
    weightsItem, weightsFeature, main.dist.obj,
    clusterAlg, distance, innerLinkage, maxK, verbose, mConsist,ml)}, mc.cores=coreCount)
    ...
    for (r in 1:repCount) {
        ...
        mCount <- connectivityMatrix(rep( 1,length(parRes[[r]][[1]][[3]])),
                                        mCount,
                                        parRes[[r]][[1]][[3]] )

    	  ml[[k]] <- connectivityMatrix(parRes[[r]][[k]],
                                      ml[[k]],
          			                      parRes[[r]][[1]][[3]] )
    }
  }
