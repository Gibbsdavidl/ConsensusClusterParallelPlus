...
calcICL = function(res, title = "untitled_consensus_cluster", plot = NULL, writeTable = FALSE) {
    ...
    for (k in 2:length(res)) {
        ...
        m = o$consensusMatrix
        m = triangle(m, mode = 2)
        for (ci in sort(unique(o$consensusClass))) {
            # each cluster in k
        ...
        # q is a matrix of k rows and sample columns, values are item consensus of sample
        # to the cluster.
        ...
        sumRes[[k]] = rankedBarPlot(q, thisColors, cc = res[[k]]$consensusClass[res[[2]]$consensusTree$order],
            paste("k=", k, sep = ""))
    }
    ...
    names(ys) = lab
    par(mfrow = c(3, 1), mar = c(4, 3, 2, 0))
    barplot(ys, col = cs, border = cs, main = "cluster-consensus", ylim = c(0, 1),
        las = 1)
    if (is.null(plot) == FALSE) {
        dev.off()
    }
    ...
    # type cci
    cci = data.frame(k = as.numeric(cci[, "k"]), cluster = as.numeric(cci[, "cluster"]),
        item = cci[, "item"], itemConsensus = as.numeric(cci[, "itemConsensus"]))

    # write to file.
    if (writeTable) {
        write.csv(file = paste(title, "/", title, ".summary.cluster.consensus.csv",
            sep = ""), row.names = F, cc)
        write.csv(file = paste(title, "/", title, ".summary.item.consensus.csv",
            sep = ""), row.names = F, cc)
    }
...
ccRun <- function(d = d, maxK = NULL, repCount = NULL, coreCount = NULL, diss = inherits(d,
  ...
  // for K=k_i
  //   keep summing over reps,
  //     Did sample i co-cluster with sample j

  Rcpp::NumericVector clusterAssignments(a);   // cluster assignment
  Rcpp::NumericMatrix bm(b);                   // mCount
  arma::mat m = Rcpp::as<arma::mat>(bm);       // mCount arma matrix
  Rcpp::NumericVector sampleKey(c);            // sample key to cluster assignment
  Rcpp::NumericVector kmax(d);                 // max K
  int kint = kmax[0];
...
sampleCols <- function(d, pSamp = NULL, pRow = NULL, weightsItem = NULL, weightsFeature = NULL) {
    ...
    ## features (if necessary) if no sampling over the features is performed, the
    ## submatrix & sample features are returned as NAs to reduce memory overhead
    ...
    if (inherits(d, "matrix")) {
        if ((!is.null(pRow)) && ((pRow < 1) || (!is.null(weightsFeature)))) {
            ## only sample the rows and generate a sub-matrix if we're sampling over the
            ## row/gene/features
            space = nrow(d)
...
CDF = function(ml, breaks = 100) {
    ...
    plot(c(0), xlim = c(0, 1), ylim = c(0, 1), col = "white", bg = "white", xlab = "consensus index",
        ylab = "CDF", main = "consensus CDF", las = 2)
    k = length(ml)
...
setClusterColors = function(past_ct, ct, colorU, colorList) {
    ...
    if (length(colorList) == 0) {
        ...
        for (tci in 1:ncol(m)) {
            ...
            if (sum(m[, tci] == maxC) == 1 & max(m[pci, ]) == maxC & sum(m[pci, ] ==
                maxC) == 1) {
                # if new column maximum is unique, same cell is row maximum and is also unique
                # Note: the greatest of the prior clusters' members are the greatest in a current
                # cluster's members.
...
clusterTrackingPlot = function(m) {
    # description: plots cluster tracking plot input: m - matrix where rows are k,
    # columns are samples, and values are cluster assignments.
    plot(NULL, xlim = c(-0.1, 1), ylim = c(0, 1), axes = FALSE, xlab = "samples",
        ylab = "k", main = "tracking plot")
    for (i in 1:nrow(m)) {
        rect(xleft = seq(0, 1 - 1/ncol(m), by = 1/ncol(m)), ybottom = rep(1 - i/nrow(m),
            ncol(m)), xright = seq(1/ncol(m), 1, by = 1/ncol(m)), ytop = rep(1 -
            (i - 1)/nrow(m), ncol(m)), col = m[i, ], border = NA)
    }
...
triangle = function(m, mode = 1) {
    # mode=1 for CDF, vector of lower triangle. mode==3 for full matrix. mode==2 for
    # calcICL; nonredundant half matrix coun mode!=1 for summary
    n = dim(m)[1]
...
rankedBarPlot = function(d, myc, cc, title) {
    ...
    barp = barplot(apply(byRank, 2, sum), col = myc[colors[, 1]], space = spaceh,
        ylim = c(0, maxH), main = paste("item-consensus", title), border = NA, las = 1)
    for (i in 2:nrow(byRank)) {
        barplot(apply(matrix(byRank[i:nrow(byRank), ], ncol = ncol(byRank)), 2, sum),
            space = spaceh, col = myc[colors[, i]], ylim = c(0, maxH), add = T, border = NA,
            las = 1)
    }
